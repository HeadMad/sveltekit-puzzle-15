export default function (level = 3) {
  ...
  let count = $derived(level * level);
  let cells = $state(Array.from({ length: count }, (_, i) => i));
  ...
  function click(index) {
    ...
    cells[iEmp] = pPrs;
    console.log({COMPLETE: cells.every((p, i) => p === i), HANDLER: typeof completeHandler === 'function'});
